...
def tile_images(np_array_of_arrays):
    ...
    :return: tiled image
    """
    assert len(np_array_of_arrays.shape) == 5 or (np_array_of_arrays.dtype == np.object and len(np_array_of_arrays.shape) == 2), 'illegal shape: np_array_of_arrays: {} '.format(np_array_of_arrays.shape)
    ...
    for np_row in range(np_array_of_arrays.shape[0]):
        ...
        row_image = np.zeros(shape=(new_rows, new_cols, 3), dtype=np.uint8)
        for np_col in range(np_array_of_arrays.shape[1]):
            if not np_array_of_arrays[np_row, np_col].shape[:2] == max_shape_for_this_row:
                wrapped_image = wrap_image_to_shape(tuple([*list(max_shape_for_this_row), 3]), np_array_of_arrays[np_row, np_col])
    ...
    for row_number, row_image in enumerate(row_images):
        if not row_image.shape[:2] == max_row_shape:
            wrapped_image = wrap_image_to_shape(tuple([*list(max_row_shape), 3]), row_image)
